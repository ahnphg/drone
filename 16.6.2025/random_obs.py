"""
drone_transfer_learning_sac_50m_random_obstacles.py

– Đầu vào: quãng đường 50m
– Huấn luyện 1 đoạn 50m, với chướng ngại vật ngẫu nhiên và tìm điểm kết thúc an toàn
– Hiển thị đường bay
"""
import argparse
import os
...
class FinalDroneEnv(gym.Env):
    def __init__(self, target_x=50.0):
        ...
        # Danh sách chướng ngại vật sẽ được sinh ngẫu nhiên trong reset
        self.obstacles = []
        # Tìm điểm kết thúc an toàn sau khi sinh chướng ngại vật trong reset
        self.target_z = 5.0  # Lưu target_z để sử dụng trong find_safe_endpoint

    def reset(self, seed=None, options=None):
        # Đặt lại trạng thái drone
        ...
        self.target_x = find_safe_endpoint(self, 50.0, self.target_z)
        self.initial_dx = self.target_x - self.drone.state[0]
        return self._get_obs(), {}

    def _generate_random_obstacles(self):
        """Sinh 2-4 chướng ngại vật ngẫu nhiên trong phạm vi 0-50m."""
        num_obstacles = np.random.randint(2, 5)  # Số lượng chướng ngại vật ngẫu nhiên
        obstacles = []
        for _ in range(num_obstacles):
            # Sinh tọa độ x ngẫu nhiên, đảm bảo không quá gần điểm bắt đầu hoặc mục tiêu
            x_start = np.random.uniform(5.0, 45.0)
            x_width = np.random.uniform(1.0, 3.0)  # Chiều rộng 1-3m
            x_end = min(x_start + x_width, 49.0)  # Không vượt quá 49m
            # Sinh tọa độ z ngẫu nhiên trong khoảng hợp lý
            z_start = np.random.uniform(4.0, 5.0)
            z_height = np.random.uniform(1.0, 2.0)  # Chiều cao 1-2m
            z_end = z_start + z_height
            obstacles.append({"x": (x_start, x_end), "z": (z_start, z_end)})
        return obstacles
...
def train_chunk(target_x, start_x, cont, load_path, save_path, log_dir):
    ...
    model.learn(total_timesteps=100_000, callback=[cb1, cb2], reset_num_timesteps=not cont)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--distance', type=int, choices=[50], required=True)
